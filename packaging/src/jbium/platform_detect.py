...
def find_browser_binary(
    ...
    if search_paths is None:
        home = Path.home()
        fetch_cache = home / ".cache" / "jbium" / "bin"
        build_out = home / "jbium" / "chromium" / "src" / "out" / "Release"

        search_paths = [
            # `jbium fetch` downloads here — the primary location for a
            # pip-installed package with no from-source build alongside it.
            fetch_cache / info.binary_name,
            fetch_cache / "Jbium.app/Contents/MacOS/jbium",  # macOS .app bundle

            # Raw build output — scripts/build_linux.sh, build_macos.sh, and
            # build_windows.bat fetch Chromium to ~/jbium/chromium/src and
            # rename the compiled binary here, for anyone building from
            # source instead of fetching a prebuilt release.
            build_out / info.binary_name,
            build_out / "Jbium.app/Contents/MacOS/jbium",
    ...
    raise FileNotFoundError(
        ...
        f"Run `jbium fetch` to download it, or build it yourself and set "
        f"binary_path in config.\n"
