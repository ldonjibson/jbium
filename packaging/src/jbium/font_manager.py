...
class FontManager:
    ...
    def __init__(self, fonts_root: Optional[str] = None):
        # Packaged manifests ship inside the installed package itself
        # (jbium/fonts/...) — parent, not parent.parent, since this file
        # now lives directly in the jbium package, not a sibling driver/
        # directory next to a repo-root fonts/ folder.
        self.fonts_root = (
            Path(fonts_root) if fonts_root
            else Path(__file__).parent / "fonts"
        )
