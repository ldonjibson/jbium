...
| 011 | AudioContext Noise | `patches/011_audio/` | ⏸️ helper-only | AudioContext/OfflineAudioContext sample fingerprinting — **not wired up**, see patch docstring |
...
## Patch 011: AudioContext Noise — NOT YET WIRED UP

**Problem:** `OfflineAudioContext` rendering (`startRendering()` +
reading the resulting `AudioBuffer` via `getChannelData()`) is a real,
actively-used fingerprinting vector this repo doesn't yet defend
against. The driver already generates and passes a
`STEALTH_AUDIO_SEED` env var (same pattern as the canvas/WebGL seeds),
but no patch reads it.

**Why it's incomplete on purpose:** every other body-replacing patch
in this list was only written after diffing the real pinned source
(`git diff tags/<version> -- <path>`) — that discipline is what caught
two real fabricated patches earlier in this project's history (canvas
`GetImage()` and geoip `EnsureUpdatedLanguage()` both invented
plausible C++ that didn't match the actual source). This patch was
drafted without server/build access to do that same check against
`AudioBuffer::getChannelData()`, and unlike `GetImage()`,
`getChannelData()` almost certainly bounds-checks the channel index —
guessing that logic wrong in a full-body replacement risks silently
removing a bounds check (a security bug reachable from plain JS), not
just a compile failure. Not a risk worth taking blind.

**What's actually done:** the noise-generation math (deterministic
per-session, sparse, ~1e-6 magnitude — inaudible, enough to change a
naive sample-hash) is self-contained and doesn't depend on any
Blink-internal knowledge, so it's written as a header-only helper.
Nothing includes or calls it yet, so this patch cannot break the
build — it only ever installs one new, unreferenced header file.

**To finish it:** see the full docstring in
`patches/011_audio/apply.sh` for the exact verification steps —
`git show tags/<version>:.../audio_buffer.cc`, confirm the real
signature and bounds-check, then wrap only the return statement
(same technique used to fix `004_canvas`'s `GetImage()`).

**Files modified:**
```
third_party/blink/renderer/platform/stealth/stealth_audio_noise.h (NEW, header-only, unreferenced)
```

**Environment variables:**
- `STEALTH_AUDIO_SEED` — already generated by the driver; not yet
  consumed by any patch
